...
This document defines the authoritative, reusable manual QA verification checklist for **CloudBackup for Windows**.
...
- [ ] Official installer executable `CloudBackup-Setup.exe`.
...
## Manual QA Verification Checklist (15 Release Gates)

All 15 verification steps below must pass cleanly before any stable release tag (`v*`) or release asset publication is permitted:

| Step | Validation Target | Verification Procedure |
|---|---|---|
| 1 | Artifact Retrieval | Download build artifact `CloudBackup-Windows-x64-Release` from GitHub Actions CI. |
...
| 9 | Non-Destructive Backup | Perform dry run and real `copy` backup of test folder; verify source files remain untouched. |
...
| 15 | Real Restore File Hash Match | Restore backup to separate staging path; verify **0 SHA-256 file mismatches**. |
